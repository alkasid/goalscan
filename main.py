...
# ── Fixtures 3 giorni ────────────────────────────────────────────────────────
# FIX: status passato separatamente per NS e per le live/FT
def get_all_fixtures():
    ...
    for date in dates:
        # Recupera NS (not started) — match futuri
        data_ns = api_get("fixtures", {"date": date, "status": "NS"})
        # Recupera live + FT — match in corso o finiti oggi
        data_ft = api_get("fixtures", {"date": date, "status": "FT"})
        data    = data_ns + data_ft
        print(f"  {date}: {len(data_ns)} NS + {len(data_ft)} FT = {len(data)} match raw")
...
# ── Ultime N gare FT nella stessa lega ───────────────────────────────────────
...
def analyze_fixture(fix):
    ...
    try:
        ...
        ko = datetime.fromtimestamp(ts, tz=timezone.utc).strftime("%H:%M")
    ...
    if not (hs["qualifies"] and as_["qualifies"]):
        ...
        return None, f"X  {' | '.join(reasons)}"

    odds_ok = has_bet365_odds(fixture_id)
    if not odds_ok:
        return None, f"OK goal ma no Bet365 — {home_name}:{hs['total']} {away_name}:{as_['total']}"
    ...
    return {
        ...
        "kickoff": ko, "date": match_date,
        "fixture_id": fixture_id, "status": match_status,
        "goals_home": goals.get("home"), "goals_away": goals.get("away")
    }, f"ALERT+BET365 | {home_name}:{hs['total']} {away_name}:{as_['total']}"
...
def _tg_send(text):
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"  [TG] Errore {r.status_code}: {r.text[:200]}")
    except Exception as e:
...
def send_telegram(matches, total_analyzed, run_date):
    ...
    if not matches:
        _tg_send(f"Nessun alert su {total_analyzed} match analizzati.")
        return

    lines = [
        f"GOAL SCAN — {run_date}",
        f"Analizzati: {total_analyzed} | Alert: {len(matches)}",
        f"Soglia: >={THRESHOLD} goal | ultime {LAST_N} gare | Bet365",
    ]
    ...
    for day in sorted(days):
        lines.append("")
        lines.append(f"--- {day_label.get(day, day)} ---")
        for m in days[day]:
            hs = m["home_stats"]; as_ = m["away_stats"]
            lines.append(f"{m['kickoff']} *{m['home']}* vs *{m['away']}*")
            lines.append(f"Casa: +{hs['scored']} -{hs['conceded']} = *{hs['total']}* | Trasferta: +{as_['scored']} -{as_['conceded']} = *{as_['total']}*")
...
def safe_write(path, content):
    """Scrive file HTML evitando UnicodeEncodeError con surrogati."""
    path.write_text(content.encode("utf-8", errors="replace").decode("utf-8"), encoding="utf-8")
...
def generate_html(matches, run_date, total_analyzed):
    ...
    day_labels = {today_str: "OGGI", d1_str: "DOMANI", d2_str: "DOPODOMANI"}

    css = (
        ":root{--bg:#080d18;--surface:#0f1623;--card:#151e2e;--accent:#00e5a0;"
        "--red:#ff4757;--orange:#ff8c00;--text:#dde3f0;--muted:#556080;--border:rgba(255,255,255,0.06);}"
        ...
        ".day-section{margin:20px 16px 0;}"
        ".day-header{font-size:1rem;font-weight:700;color:var(--text);"
        "padding:8px 0 10px;border-bottom:1px solid var(--border);margin-bottom:12px;"
        "display:flex;align-items:center;gap:10px;}"
        ".day-count{font-size:.72rem;color:var(--muted);background:rgba(255,255,255,0.05);"
        "padding:2px 10px;border-radius:100px;}"
        ".ts{margin-bottom:14px;}"
        ".th{display:flex;align-items:center;gap:12px;margin-bottom:10px;}"
        ".tl{font-size:1.05rem;font-weight:700;color:var(--accent);}"
        ...
        ".bet{font-size:.6rem;color:var(--accent);text-align:center;margin-top:7px;opacity:.6;}"
    ...
    def make_card(m):
        ...
        lg = f'{m["league"]} - {country}' if country and country != "World" else m["league"]
        return (
            ...
            f'</div></div></div>'
            f'<div class="bet">BET365 VERIFIED</div></div>'
        )
    ...
    if not matches:
        ...
        for day in sorted(days):
            ...
            day_html  = (
                ...
                f'<span class="day-count">{day_total} alert</span></div>'
            )
    ...
    return (
        ...
        f'<header><div>'
        f'<div class="htitle">&#9917; GoalScan - {run_date}</div>'
        f'<div class="hsub">Entrambe >={THRESHOLD} goal (fatti+subiti) ultime {LAST_N} gare stessa lega + Bet365'
        f' - {total_analyzed} match analizzati</div>'
        f'</div><div class="hbadge">{len(matches)} ALERT</div></header>'
        f'<div class="cbar">'
        f'<span>Soglia: <strong>>={THRESHOLD}</strong></span>'
        ...
        f'<span>Copertura <strong>3 giorni</strong></span>'
        f'</div>'
...
def main():
    ...
    if not all_fixtures:
        ...
        safe_write(out, generate_html([], run_date, 0))
        return
    ...
    print(f"\n{'='*60}")
    print(f"ALERT (goal + Bet365): {len(qualified)} / {total}")
    print(f"{'='*60}")
    ...
    out = docs / "index.html"
    safe_write(out, generate_html(qualified, run_date, total))
    print(f"\nReport salvato: {out}")

    # Salva lista fixture_id per eventuale uso futuro
    ids      = [m["fixture_id"] for m in qualified if m.get("fixture_id")]
    ids_file = docs / "alert_ids.json"
    existing = []
    if ids_file.exists():
        try: existing = json.loads(ids_file.read_text())
        except: pass
    ids_file.write_text(json.dumps(list(set(existing + ids))))
    print(f"alert_ids.json: {len(ids)} nuovi fixture")
